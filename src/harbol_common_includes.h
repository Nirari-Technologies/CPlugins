...
static inline void *harbol_recalloc(void *const restrict arr, const size_t new_size, const size_t element_size, const size_t old_size)
{
	/**
		...
		Unless your program does a few huge allocations and tons of little deallocations (~200 times more deallocations than allocations, which can be possible), you should use malloc()/free().
	 */
	uint8_t *const restrict newdata = harbol_realloc(arr, new_size * element_size);
	if( newdata==NULL ) {
		return arr;
	} else {
...
static inline NO_NULL uint8_t *make_buffer_from_binary_wlen(const char file_name[restrict], size_t *const bytes)
{
	FILE *restrict file = fopen(file_name, "rb");
	if( file==NULL ) {
		return NULL;
...
static inline NO_NULL char *make_buffer_from_text(const char file_name[restrict])
{
	...
	if( file==NULL ) {
		...
		if( filesize<=0 ) {
			...
			char *restrict stream = harbol_alloc(filesize + 1, sizeof *stream);
			const size_t bytes_read = fread(stream, sizeof *stream, filesize, file);
			fclose(file), file=NULL;
...
static inline NO_NULL char *make_buffer_from_text_wlen(const char file_name[restrict], size_t *const len)
{
	FILE *restrict file = fopen(file_name, "r");
	if( file==NULL ) {
		return NULL;
